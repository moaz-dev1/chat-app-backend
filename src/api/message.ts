...
messageRoutes.post('/', authToken, async (req: Request, res: Response) => {
    try {
        ...
        const result = await pool.query('INSERT INTO messages (sender_id, receiver_id, content, sent_time, room_id) VALUES($1, $2, $3, $4, $5) RETURNING id',
            [newMessage.sender.id, newMessage.receiver.id, newMessage.content, newMessage.sentTime, newMessage.room.id]
        );

        await pool.query('UPDATE rooms SET last_message_id = $1 WHERE rooms.id = $2', [result.rows[0].id, newMessage.room.id]);
